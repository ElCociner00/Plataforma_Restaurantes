-- Fuente historica convertida realmente de mojibake CP1252 a UTF-8.
-- Reemplaza los cuerpos remotos sin modificar filas.
-- ============================================================================
-- CICLO DE VIDA � FASE C (d) � Guarda aplicada a los RPC de escritura
--
-- Estas dos funciones se reproducen ENTERAS tal y como est�n hoy en la base,
-- con una �nica l�nea a�adida: la llamada a exigir_acceso_escritura(), justo
-- despu�s de que cada una resuelva su empresa y compruebe que no es NULL.
--
-- Se hace as�, y no con una sustituci�n por expresi�n regular sobre
-- pg_get_functiondef(), porque subir_cierre_turno tiene 18.795 caracteres y un
-- fallo de coincidencia dejar�a el cierre de turno inservible.
-- ============================================================================
...
BEGIN
  ...
  -- Corta la escritura solo si la cuenta nunca activ� su prueba o est� dada de
  -- baja. La mora NO bloquea: eso sigue en observaci�n (�2.4 del plan).
