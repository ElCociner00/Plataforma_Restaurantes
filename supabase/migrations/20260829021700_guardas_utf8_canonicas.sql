-- Segunda aplicacion canonica UTF-8, conservada para cerrar la divergencia
-- detectada por las aserciones remotas.
-- ============================================================================
-- CICLO DE VIDA - FASE C (d) - Guarda aplicada a los RPC de escritura
--
-- Estas funciones se reproducen enteras desde la definicion canonica.
-- La linea funcional anadida llama a exigir_acceso_escritura() despues de
-- resolver la empresa y comprobar que no es NULL.
--
-- Se usa una definicion completa, no una sustitucion por expresion regular de
-- pg_get_functiondef(), porque subir_cierre_turno tiene 18.795 caracteres y un
-- modo que un fallo de coincidencia no deje el cierre inservible.
-- ============================================================================
...
BEGIN
  ...
  -- Guarda de acceso de ciclo de vida.
  -- Corta la escritura si la cuenta no activo su prueba o esta dada de baja.
  -- La mora no bloquea mientras esa politica siga deshabilitada.
