-- Primer intento conservado por trazabilidad: el archivo fuente tambien tenia
-- mojibake y por eso esta version fue reemplazada por 20260829021500.
-- ============================================================================
-- CICLO DE VIDA · FASE C (d) — Guarda aplicada a los RPC de escritura
--
-- Estas dos funciones se reproducen ENTERAS tal y como están hoy en la base,
-- con una única línea añadida: la llamada a exigir_acceso_escritura(), justo
-- después de que cada una resuelva su empresa y compruebe que no es NULL.
--
-- Se hace así, y no con una sustitución por expresión regular sobre
-- pg_get_functiondef(), porque subir_cierre_turno tiene 18.795 caracteres y un
-- fallo de coincidencia dejaría el cierre de turno inservible.
-- ============================================================================
...
BEGIN
  ...
  IF v_empresa IS NULL THEN
    RAISE EXCEPTION 'Tu cuenta no estÃ¡ vinculada a ninguna empresa'
      USING ERRCODE = '42501';
  END IF;

  -- [guarda de acceso · ciclo de vida]
  -- Corta la escritura solo si la cuenta nunca activó su prueba o está dada de
  -- baja. La mora NO bloquea: eso sigue en observación (§2.4 del plan).
  PERFORM public.exigir_acceso_escritura(v_empresa);


  -- â”€â”€ Datos obligatorios â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  IF v_numero NOT IN (1, 2, 3) THEN
    RAISE EXCEPTION 'Indica la jornada del turno: 1 (maÃ±ana), 2 (tarde) o 3'
      USING ERRCODE = '22023';
  END IF;
  ...
  -- â”€â”€ 1. Idempotencia: Â¿este envÃ­o ya se procesÃ³? â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  IF v_token IS NOT NULL THEN
    ...
    IF v_existentes > 0 THEN
      RETURN jsonb_build_object(
        'ok', true,
        'message', 'Este cierre ya se habÃ­a guardado.',
  ...
  -- â”€â”€ 2. Â¿Ya hay un turno con esta jornada? â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  IF v_es_local THEN
    SELECT count(*) INTO v_existentes
    FROM public.cierres_turno_final_locales
  ...
  IF v_existentes > 0 THEN
    ...
    IF NOT public.app_es_admin() AND v_fecha <> v_hoy THEN
      RAISE EXCEPTION 'Solo puedes reemplazar turnos del dÃ­a en curso. Pide a un administrador que lo haga.'
        USING ERRCODE = '42501';
    END IF;
    ...
    -- Un lote por sobrescritura: es lo que permite a la pantalla de auditorÃ­a
    -- mostrar el reemplazo como una unidad y devolverlo si fue un error.
    v_lote := gen_random_uuid();
    v_observacion := format(
      'VersiÃ³n anterior del cierre del %s (jornada %s), reemplazada el %s por %s. %s',
      ...
      CASE WHEN NULLIF(v_motivo, '') IS NULL
           THEN 'No se indicÃ³ motivo.'
           ELSE 'Motivo indicado: ' || v_motivo END
    );

    -- â”€â”€ 3. Mover la versiÃ³n anterior al histÃ³rico â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    IF v_es_local THEN
  ...
  -- â”€â”€ 4. Efectivo de apertura, calculado aquÃ­ â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  -- â”€â”€ 5. Insertar el cierre â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  IF v_es_local THEN
  ...
  -- â”€â”€ 6. Las dos filas del efectivo de apertura â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  IF v_es_local THEN
    INSERT INTO public.cierres_turno_final_locales (
      empresa_id, fecha_turno, numero_turno, token_envio, responsable_id,
      comentarios, valor, hora_inicio, hora_fin, variable, registrado_por,
  ...
  -- â”€â”€ 7. Apoyos â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  RETURN jsonb_build_object(
    ...
    'message', CASE WHEN v_movidas > 0
                    THEN 'Cierre reemplazado. La versiÃ³n anterior quedÃ³ en el histÃ³rico.'
...
BEGIN
  ...
  IF v_empresa IS NULL THEN
    RAISE EXCEPTION 'Tu cuenta no estÃ¡ vinculada a ninguna empresa' USING ERRCODE = '42501';
  END IF;
  ...
  IF jsonb_array_length(v_parametros) = 0 THEN
    RAISE EXCEPTION 'No se recibiÃ³ ningÃºn parÃ¡metro que guardar' USING ERRCODE = '22023';
  END IF;
