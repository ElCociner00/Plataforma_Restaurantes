...
## Parche posterior 11 — 2026-08-15 — Render de vista resumida del histórico de nómina

### 1. Objetivo de la petición

Corregir el módulo `nomina/historico.html` para que renderice correctamente la respuesta exitosa del webhook `https://n8n.enkrato.com/webhook/nomina_historico_consultar_vista` cuando n8n devuelve un arreglo envoltorio con la forma `[{ "data": [ ...nóminas... ] }]`. Antes de este parche la consulta llegaba con HTTP 200, pero el normalizador tomaba el objeto envoltorio como si fuera una nómina única, por lo que no encontraba `id`, `fecha` ni `periodo` en las filas reales y el listado quedaba sin los registros esperados.

### 2. Archivos implicados y tipo de modificación

- `js/nomina_historico.js`: modificación aislada del normalizador de respuesta. Se añadió `extractHistoricoRows(payload)` para aplanar respuestas directas (`[...]`), respuestas con objeto envoltorio (`{ data: [...] }`) y respuestas con arreglo envoltorio (`[{ data: [...] }]`). `normalizeHistoricoRows(payload)` ahora usa esa extracción antes de construir filas renderizables y descarta únicamente objetos que no tengan señales mínimas de nómina (`id`, fecha o periodo). No se modificaron archivos vitales de login, sesión, contexto ni header.
- `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_11_parches.md`: documentación de este parche posterior, anexada al documento histórico del cambio grande de nómina y creada desde `..._10_parches.md` para mantener la secuencia solicitada.

### 3. Notas de emergencia para revertir

Si el backend cambia y se necesita volver al comportamiento anterior:

1. En `js/nomina_historico.js`, eliminar completo el bloque:
...
2. En el mismo archivo, reemplazar el bloque actual de `normalizeHistoricoRows` por la versión anterior:
...
3. Si se revierte la documentación, borrar `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_11_parches.md` y mantener como último documento `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_10_parches.md`.

### 4. Guía para exportar este parche a otro repositorio

Se realizaron cambios puntuales, no masivos. Para exportarlos correctamente:

1. Copiar primero el cambio de `js/nomina_historico.js`, específicamente la nueva función `extractHistoricoRows(payload)` y la versión actualizada de `normalizeHistoricoRows(payload)`.
2. Verificar que el repositorio destino mantenga las URLs centralizadas como este proyecto en `js/webhooks.js`. Este parche no crea endpoints nuevos: usa `WEBHOOK_NOMINA_HISTORICO_VISTA`, que debe apuntar a `https://n8n.enkrato.com/webhook/nomina_historico_consultar_vista` o a su equivalente del ambiente destino.
3. Confirmar que la página destino tenga `tbody id="historicoNominaBody"` y cargue el módulo aislado `js/nomina_historico.js` desde `nomina/historico.html`.
4. Validar que no exista otro normalizador que ya aplane `[{ data: [...] }]`. Si existe, priorizar una sola función para evitar duplicidad y mantener funcionalidad máxima.
5. Ejecutar `node --check js/nomina_historico.js` y probar manualmente una respuesta real o simulada con la forma `[{ data: [{ id, empresa_id, responsable_id, fecha, periodo }] }]`.
6. No copiar ni tocar módulos de login, sesión, contexto o header. Este parche depende de `buildRequestHeaders`, `getUserContext` y `listAvailableLocalContexts`, pero no modifica esas funciones.

### 5. Check funcional para logs

- Histórico nómina — consulta vista: funciona con HTTP 200 y respuesta envoltorio `[{ data: [...] }]`.
- Histórico nómina — render listado: corregido; cada item interno de `data` se convierte en fila de la tabla.
- Histórico nómina — filtros por fecha/responsable/sede: se conservan sin cambios; aplican sobre las filas ya aplanadas.
- Histórico nómina — detalle al seleccionar fila: se conserva sin cambios; usa el `id` real de cada nómina renderizada.
- Histórico nómina — borrado: se conserva sin cambios; usa el `id` real de cada nómina renderizada.
- Login, sesión, contexto y header: no modificados.

## Parche posterior 12 — 2026-08-15 — Sede segura y submódulo de detalle histórico

### 1. Objetivo de la petición

Corregir dos puntos del módulo Histórico Nómina sin tocar archivos vitales de login, sesión, contexto o header: primero, evitar que la columna `Empresa / sede` exponga UUIDs como `f37f6983-9d59-40c8-b0c1-5949b45743c6`, resolviendo el nombre de la sede cuando exista en los locales disponibles y mostrando `Sede sin resolver` cuando no haya nombre seguro; segundo, crear un submódulo separado para el detalle de una nómina histórica, abierto desde el listado mediante el `row id`/`id` de la nómina, para consultar y renderizar encabezado, totales, detalles, ingresos, deducciones y parámetros.

### 2. Archivos implicados y tipo de modificación

- `js/nomina_historico.js`: se modificó de forma aislada el render del listado. Se agregaron validadores `isUuidLike`, `cleanSedeName` y resolución por `state.locales` para que `getSedeHistorico(row)` nunca use `empresa_id`, `tenant_id` ni `consultado` como texto visible si son UUIDs. También se cambió el click de una fila para abrir `nomina/historico_detalle.html?id=<id>` en lugar de renderizar el detalle dentro del mismo panel.
- `nomina/historico.html`: se retiró el panel de detalle embebido que ya no se usa y se actualizó el mensaje de estado para dejar claro que el detalle se abre en otro submódulo/página.
- `nomina/historico_detalle.html`: archivo nuevo y aislado. Define la vista del submódulo de detalle, carga los estilos existentes `main.css` y `nomina.css`, mantiene el header/footer existentes sin modificarlos y carga únicamente `js/nomina_historico_detalle.js` para consultar el detalle.
- `js/nomina_historico_detalle.js`: archivo nuevo y aislado. Lee `id` desde la URL, envía `id`, `nomina_id` y `row_id` al webhook centralizado `WEBHOOK_NOMINA_HISTORICO_RENDERIZAR`, normaliza la respuesta esperada y renderiza tarjetas de resumen y tablas para `totales`, `detalles`, `ingresos`, `deducciones`, `parametros` y `locales`.
- `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_12_parches.md`: documentación de este parche posterior, creada a partir del documento de 11 parches para conservar el historial y la secuencia solicitada.

### 3. Notas de emergencia para revertir

1. En `js/nomina_historico.js`, para volver al comportamiento anterior inseguro, reemplazar el bloque completo de resolución de sede (`uuidPattern`, `isUuidLike`, `resolveLocalName`, `cleanSedeName` y `getSedeHistorico`) por la línea anterior:
   ...
   No se recomienda porque puede volver a exponer UUIDs de empresa/sede en la tabla.

2. En `js/nomina_historico.js`, para volver al detalle embebido, reemplazar el bloque del click de fila:
   ...
   por la llamada anterior a `renderDetalle(row)`. Solo hacerlo si también se restauran las funciones y elementos HTML del panel embebido.

3. En `nomina/historico.html`, para restaurar el panel embebido, volver a añadir antes de `historicoNominaStatus` el bloque con `id="historicoNominaDetallePanel"`, `historicoNominaVolver`, `historicoNominaDetalleTitulo` y `historicoNominaDetalleResumen`.

4. Si hay emergencia con el submódulo nuevo, borrar `nomina/historico_detalle.html` y `js/nomina_historico_detalle.js` únicamente después de revertir el click del listado; de lo contrario el usuario quedaría navegando a una página inexistente.

5. Si se revierte la documentación, borrar `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_12_parches.md` y conservar como último documento `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_11_parches.md`.

### 4. Guía para exportar este parche a otro repositorio

Se añadieron archivos nuevos y cambios puntuales. Para exportar correctamente:

1. Migrar primero la centralización de URLs del repositorio destino. Este proyecto toma endpoints desde `js/webhooks.js`; debe existir una constante equivalente a `WEBHOOK_NOMINA_HISTORICO_RENDERIZAR` apuntando al webhook de detalle, por ejemplo `https://n8n.enkrato.com/webhook/nomina_historico_consultar`.
2. Copiar los cambios de `js/nomina_historico.js` relacionados con resolución segura de sede y navegación a `historico_detalle.html?id=<id>`. Verificar que el listado ya tenga `tbody id="historicoNominaBody"` y que los objetos recibidos tengan `id`.
3. Copiar los archivos nuevos `nomina/historico_detalle.html` y `js/nomina_historico_detalle.js`. Estos archivos se conectan así: el HTML define el contenedor visual del submódulo y el JS lee el ID desde querystring, consulta el webhook de detalle y renderiza las tablas.
4. Verificar si el repositorio destino ya tiene un detalle histórico. Si existe, priorizar integrar la consulta por `row_id` en ese submódulo existente para evitar duplicidad, manteniendo la regla de no exponer UUIDs como nombres visibles.
5. No copiar ni modificar login, sesión, contexto ni header. Este parche depende de `buildRequestHeaders`, `getUserContext` y `listAvailableLocalContexts`, pero no los altera.
6. Ejecutar `node --check js/nomina_historico.js` y `node --check js/nomina_historico_detalle.js`. Luego probar en navegador: abrir el histórico, confirmar que `Empresa / sede` no muestre UUIDs y hacer click en una nómina para validar que se abre `historico_detalle.html?id=<id>`.

### 5. Check funcional para logs

- Histórico nómina — listado: funciona y mantiene las filas renderizadas del webhook de vista.
- Histórico nómina — columna Empresa / sede: corregida; ya no muestra UUIDs como texto visible y resuelve nombres desde locales disponibles cuando existen.
- Histórico nómina — sede no resoluble: funciona de forma segura mostrando `Sede sin resolver` en vez de exponer el ID.
- Histórico nómina — click en fila: ahora abre un submódulo separado `historico_detalle.html?id=<id>`.
- Histórico nómina — detalle: nuevo submódulo consulta por `id`/`nomina_id`/`row_id` y renderiza encabezado, totales, detalles, ingresos, deducciones, parámetros y locales.
- Histórico nómina — borrado: se conserva en el listado y no cambia su webhook.
- Login, sesión, contexto y header: no modificados.

## Parche posterior 13 — 2026-08-16 — Detalle histórico legible y sin selector de local

### 1. Objetivo de la petición

Corregir el segundo paso del Histórico Nómina: al hacer click sobre una nómina histórica, el submódulo de detalle debe abrirse sin disparar la selección de local, porque solo se está consultando una nómina ya guardada en el contexto actual. Además, el detalle debe parecerse a las tablas del módulo de nómina original: nombres legibles sin `_`, campos técnicos ocultos, IDs no expuestos, monedas con formato colombiano, horas separadas de valores monetarios y diferenciación visual entre turnos normales y apoyos.

### 2. Archivos implicados y tipo de modificación

- `nomina/historico_detalle.html`: se retiró la carga de `js/local_context_navigation_guard.js` solo en este submódulo nuevo. Esto lo deja como excepción aislada que conserva el local actual y evita desplegar la subpantalla de selección de local al entrar al detalle histórico. No se modificó el guard global ni archivos de sesión/contexto.
- `js/nomina_historico_detalle.js`: se reemplazó el render genérico por render específico de nómina. Ahora `labelize()` cambia `_` por espacios y capitaliza títulos; `renderResumenTotales()` compara horas contra valores monetarios; `renderDetalles()` replica una tabla de detalle con tipo, sede resuelta, fecha, día, horario, horas y propinas; `renderCalculos()` separa los valores monetarios anidados de `calculos`; `renderConceptos()` elimina columnas técnicas como `estado`, `fuente`, `naturaleza` y cambia `empleado_nombre` por `Responsable`; `renderParametros()` ordena columnas como `Concepto`, `Valor`, `Unidad` y elimina `row_id`/duplicados.
- `js/nomina_historico_detalle.js`: se fortaleció la resolución de sede para no mostrar UUIDs en detalle de turnos ni resumen. Si no hay nombre confiable, muestra `Sede sin resolver`.
- `css/nomina.css`: se agregaron estilos acotados al detalle histórico para que sus tablas usen ancho horizontal, eviten cortes silábicos feos y aprovechen el espacio disponible sin romper la gama visual existente.
- `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_13_parches.md`: documentación de este parche posterior, creada desde el documento de 12 parches para conservar el historial y la secuencia solicitada.

### 3. Notas de emergencia para revertir

1. Para reactivar la selección de local en el submódulo de detalle, volver a añadir en `nomina/historico_detalle.html`, debajo de `header.js`, la línea:

   ```html
   <script type="module" src="../js/local_context_navigation_guard.js"></script>
   ```

   No se recomienda para este submódulo, porque el detalle histórico no cambia de local y debe conservar el contexto actual.

2. Para volver al render genérico anterior, en `js/nomina_historico_detalle.js` restaurar las funciones previas `renderTable()`, `objectToRows()` y el arreglo de tablas que incluía `Locales`. Esto regresará los JSON anidados visibles y columnas técnicas; usar solo como contingencia temporal.

3. Para revertir la tabla comparativa de `Resumen de totales`, reemplazar `renderResumenTotales(row.totales)` por una tabla genérica de `Object.entries(row.totales)`. No se recomienda porque vuelve a mezclar horas y valores monetarios verticalmente.

4. Para revertir los estilos visuales, eliminar al final de `css/nomina.css` el bloque marcado como `Parche histórico nómina 2026-08-16: detalle legible, ancho y sin IDs visibles`.

5. Si se revierte la documentación, borrar `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_13_parches.md` y conservar como último documento `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_12_parches.md`.

### 4. Guía para exportar este parche a otro repositorio

1. Migrar `nomina/historico_detalle.html` verificando que no cargue el guard/preselector de local. Si el repositorio destino tiene un guard equivalente, agregar este submódulo a sus excepciones o retirar su script solo de esta página.
2. Migrar `js/nomina_historico_detalle.js` completo o, si ya existe un detalle histórico, copiar las funciones específicas: `labelize`, `renderResumenTotales`, `renderDetalles`, `renderCalculos`, `renderConceptos`, `renderParametros`, `resolveSedeNames` y `resolveDetalleSede`.
3. Confirmar que el destino centralice URLs como este repositorio en `js/webhooks.js` y que `WEBHOOK_NOMINA_HISTORICO_RENDERIZAR` apunte al webhook de detalle histórico.
4. Verificar que ningún render use directamente `empresa_id`, `tenant_id`, `responsable`, `row_id`, `duplicado`, `incluido`, `incluidoTransporte`, `estado`, `fuente` o `naturaleza` como columna visible para cliente.
5. Migrar el bloque CSS acotado de `css/nomina.css` para evitar cortes tipo `jue-ve-s` y columnas angostas en tablas históricas.
6. Ejecutar `node --check js/nomina_historico_detalle.js` y probar con una nómina que tenga turnos normales, apoyos, ingresos, deducciones y parámetros.

### 5. Check funcional para logs

- Histórico nómina — detalle: abre sin disparar selector/preselector de local.
- Histórico nómina — resumen de totales: funciona como tabla comparativa horas vs valores monetarios, con nombres sin `_`.
- Histórico nómina — detalles: oculta `row_id`, `duplicado`, `incluido`, `incluidoTransporte`, `responsable` ID y `sede_nombre` duplicada.
- Histórico nómina — cálculos: desglosa el JSON `calculos` en columnas monetarias con formato COP.
- Histórico nómina — ingresos/deducciones: elimina columnas técnicas, normaliza unidades y limita decimales de moneda/cantidades.
- Histórico nómina — parámetros: ordena como `Concepto`, `Valor`, `Unidad` y elimina columnas duplicadas/técnicas.
- Histórico nómina — apoyos vs turnos normales: diferenciación visual conservada con clases existentes `nomina-row-apoyo` y `nomina-row-turno`.
- Login, sesión, contexto, header y guard global: no modificados.

## Parche posterior 14 — 2026-08-18 — Mensajes, detalle histórico y parámetros tiempo locales

### 1. Objetivo de la petición

Corregir detalles finales del flujo Histórico Nómina y añadir una mejora aislada al módulo principal de Nómina. En histórico, el borrado debe mostrar mensajes simples sin exponer endpoints, las tablas del detalle deben mejorar formato visual/textual y las deducciones deben leerse y guardarse de forma más confiable. En Nómina principal, los usuarios necesitan guardar sus parámetros de tiempo modificados como predeterminados locales del navegador mediante `localStorage`, sin crear tablas de base de datos ni tocar archivos vitales.

### 2. Archivos implicados y tipo de modificación

- `js/nomina_historico.js`: se simplificó el mensaje exitoso de borrado a `Borrado exitosamente.` y el error ya no menciona ni expone la URL del webhook; ahora solo indica que falló la conexión al intentar borrar.
- `js/nomina_historico_detalle.js`: se ajustó `labelize()` para bajar todo a minúscula antes de capitalizar y evitar errores visuales como `SÁBado` o `MiÉRcoles`. También se añadió `tdStrong()` para que las columnas `Tipo` y `Sede` aparezcan en negrita en `Detalles` y `Detalles cálculos`, replicando el énfasis del módulo de nómina original.
- `js/nomina_historico_detalle.js`: se reforzó la lectura de deducciones. Si `tablas.deducciones` no llega como arreglo visible, el detalle intenta leer `deducciones_ley` y deducciones auxiliares dentro de `auxiliares.deducciones` antes de mostrar la tabla vacía.
- `nomina/index.html`: se agregó el botón `Guardar parámetros de tiempo` debajo de la tabla `Parámetros tiempo`.
- `js/nomina.js`: se añadió `PARAMETROS_TIEMPO_STORAGE_KEY`, `loadParametrosTiempoDefaults()` y `guardarParametrosTiempoNomina()` para guardar/leer predeterminados de `parametrosTiempo` desde `localStorage`. También se agregó el listener del botón nuevo y la carga de predeterminados durante `init()`.
- `js/nomina.js`: se ajustó el payload de histórico para usar una variable `deduccionesHistorico`; si `state.movimientos` ya contiene deducciones, las guarda desde allí, y si no existen usa fallback con deducciones de ley y auxiliares calculadas. Esto ayuda a diagnosticar si el problema era guardado o lectura.
- `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_14_parches.md`: documentación de este parche posterior, creada desde el documento de 13 parches para conservar el historial y la secuencia solicitada.

### 3. Notas de emergencia para revertir

1. En `js/nomina_historico.js`, para volver a los mensajes anteriores, reemplazar `Borrado exitosamente.` por el texto largo anterior y restaurar el catch que incluía el endpoint. No se recomienda porque exponer URLs de webhook en frontend no es necesario.
2. En `js/nomina_historico_detalle.js`, si se desea revertir el formato de días, quitar `.toLocaleLowerCase("es-CO")` de `labelize()`. No se recomienda porque puede volver a mostrar mezclas de mayúsculas/minúsculas.
3. En `js/nomina_historico_detalle.js`, para revertir la negrita, eliminar `tdStrong()` y volver a mapear todas las celdas con `td` en `renderDetalles()` y `renderCalculos()`.
4. En `nomina/index.html`, borrar el botón:

   ```html
   <button type="button" id="guardarParametrosTiempoNomina">Guardar parámetros de tiempo</button>
   ```

5. En `js/nomina.js`, eliminar `guardarParametrosTiempoBtn`, `PARAMETROS_TIEMPO_STORAGE_KEY`, `loadParametrosTiempoDefaults()`, `guardarParametrosTiempoNomina()`, la llamada `loadParametrosTiempoDefaults();` en `init()` y el listener `guardarParametrosTiempoBtn?.addEventListener(...)`.
6. En `js/nomina.js`, si se revierte la mejora de deducciones históricas, volver a usar directamente `state.movimientos.filter(...)` en `tablas.deducciones` y `buildDeduccionesLeyRows(calculation)` en `tablas.deducciones_ley`.
7. Si se revierte la documentación, borrar `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_14_parches.md` y conservar como último documento `docs/2026-07-16_ajustes_nomina_historico_y_detalles_y_13_parches.md`.

### 4. Guía para exportar este parche a otro repositorio

1. Migrar primero la centralización de URLs existente en `js/webhooks.js`; este parche no crea endpoints nuevos.
2. Copiar los cambios de mensajes de `js/nomina_historico.js` para no exponer endpoints en errores de borrado.
3. Copiar los cambios de `js/nomina_historico_detalle.js` relacionados con `labelize`, `tdStrong`, lectura fallback de deducciones y render de `Tipo`/`Sede` en negrita.
4. En el módulo principal de nómina del repositorio destino, verificar que exista una tabla con `tbody id="nominaParametrosTiempoBody"`. Añadir debajo el botón `guardarParametrosTiempoNomina` y copiar las funciones de `localStorage`.
5. Validar que el destino no tenga una base de datos para parámetros de tiempo de interfaz. Si sí existe una persistencia oficial, usar esa antes de `localStorage`; si no existe, mantener esta solución aislada por navegador.
6. Validar que el guardado histórico incluya deducciones dentro de `tablas.deducciones`. Si en el destino las deducciones llegan por otra clave, adaptar solo la construcción de `deduccionesHistorico` y el normalizador de detalle.
7. Ejecutar `node --check js/nomina.js`, `node --check js/nomina_historico.js` y `node --check js/nomina_historico_detalle.js` antes de desplegar.

### 5. Check funcional para logs

- Histórico nómina — borrado exitoso: muestra `Borrado exitosamente.`.
- Histórico nómina — error de borrado: no expone URL de webhook; muestra error de conexión.
- Histórico nómina — detalle columnas Tipo/Sede: aparecen en negrita en detalles y cálculos.
- Histórico nómina — día: se normaliza a primera mayúscula y resto minúscula.
- Histórico nómina — deducciones: ahora intenta leer `deducciones`, `deducciones_ley` y auxiliares como fallback.
- Nómina principal — parámetros tiempo: se pueden guardar predeterminados en `localStorage` desde el botón nuevo.
- Nómina principal — histórico guardado: deducciones se incluyen desde movimientos o fallback calculado.
- Login, sesión, contexto, header y guard global: no modificados.
