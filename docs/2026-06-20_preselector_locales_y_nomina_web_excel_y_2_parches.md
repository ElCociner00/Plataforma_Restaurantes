...
# Parche posterior 2 - Apoyos de nómina y distribución manual de compras por local

## 1. Objetivo del parche

Añadir dos mejoras operativas:

1. Incorporar la nueva tabla de `apoyos` recibida por la consulta de nómina para que se vea en la interfaz web y se exporte en el Excel del empleado. Esta tabla permite validar tiempos de apoyo, propinas y reconocer esos tiempos como ingreso adicional configurable.
2. Agregar en compras un submódulo `Distribuir locales` para que administradores u operativos definan manualmente si una factura pertenece al local actual o debe enviarse a otro local mediante webhook, sin abrir el detalle de productos.

## 2. Archivos implicados en este parche

### `nomina/index.html` (modificado)

- Se agregó una tabla visual `Apoyos` con columnas limpias para cliente: `fecha turno`, `responsable turno`, `apoyo`, `tiempo`, `hora inicio`, `hora fin` y `propina`.
- La tabla incluye controles de validación para que el administrador decida si un apoyo entra o no en el cálculo.

### `js/nomina.js` (modificado)

- Se agregó estado `apoyosDetalle` para conservar las filas de apoyo del webhook.
- Se añadieron utilidades para convertir minutos a `HH:MM` y volver de `HH:MM` a minutos.
- Se normalizan los nombres de responsables usando la lista de responsables cargada por el módulo, evitando mostrar identificadores técnicos al cliente cuando existe nombre asociado.
- `buildPayrollRowsFromEditableDetail()` suma horas de apoyo válidas como `Horas de apoyo`, usando tarifa de hora extra si existe o, como respaldo, la tarifa de hora diurna común.
- Se suman las propinas válidas como `Propinas de apoyo`.
- `renderApoyos()` pinta una tabla editable y profesional, sin mencionar detalles técnicos como webhook o metadatos.
- El Excel generado por `Descargar Excel empleado` ahora incluye una hoja adicional `Apoyos` con la misma información normalizada.

### `css/nomina.css` (modificado)

- Se añadieron estilos para la tabla de apoyos y sus inputs de edición.

### `compras/index.html` (modificado)

- Se agregó una cuarta pestaña `Distribuir locales`, ubicada junto a `Módulo principal`, `No corresponde` y `Revisadas`.

### `js/compras.js` (modificado)

- Se incorporó `listAvailableLocalContexts()` para listar empresa principal/locales disponibles.
- Se agregó lógica de submódulo de distribución: selector de local destino, botón `Enviar a local` y botón `Actual`.
- `Enviar a local` llama a `WEBHOOK_COMPRAS_REASIGNAR_LOCAL` con empresa matriz, empresa actual, tenant origen, tenant destino, datos de factura y usuario.
- `Actual` marca la factura localmente como perteneciente al contexto actual para retirarla de la pestaña de distribución y dejarla gestionable en el flujo principal.
- Se conserva el detalle/match tradicional intacto para `Módulo principal`.

### `css/compras.css` (modificado)

- Se agregaron estilos para el estado visual de facturas distribuidas y para el panel manual de selector local/botones.

### `js/webhooks.js` (modificado)

- Se centralizó `WEBHOOK_COMPRAS_REASIGNAR_LOCAL` para la reasignación manual de facturas entre locales.

### `docs/2026-06-20_preselector_locales_y_nomina_web_excel_y_2_parches.md` (renombrado/modificado)

- Se renombró el documento acumulado a `y_2_parches` y se agregó este segundo parche posterior.

## 3. Reversión de emergencia del parche 2

### Revertir apoyos en nómina

1. En `nomina/index.html`, eliminar el bloque `nomina-apoyos-panel` completo.
2. En `js/nomina.js`, retirar:
   - `apoyosBody`.
   - `state.apoyosDetalle`.
   - `minutesToHourText()` y `hoursToMinutes()` si no se usan en otra parte.
   - `renderApoyos()`.
   - El listener `apoyosBody?.addEventListener("change", ...)`.
   - Las filas `Horas de apoyo` y `Propinas de apoyo` dentro de `buildPayrollRowsFromEditableDetail()`.
   - La hoja `Apoyos` dentro de `buildExcelHtml()`.
3. En `css/nomina.css`, eliminar `.nomina-apoyos-panel` y reglas relacionadas.

### Revertir distribución manual en compras

1. En `compras/index.html`, eliminar el botón `tabComprasLocales`.
2. En `js/compras.js`, retirar:
   - Import de `listAvailableLocalContexts` si queda sin uso.
   - Import de `WEBHOOK_COMPRAS_REASIGNAR_LOCAL`.
   - Estado `localContexts`, `DISTRIBUCION_ACTUAL_KEY` y helpers de localStorage/distribución.
   - Rama `view === "locales"` en `renderFacturas()`.
   - Funciones `ensureLocalContexts()`, `reasignarFacturaLocal()` y `marcarFacturaActual()`.
   - Listener de `tabLocales` en `init()`.
3. En `css/compras.css`, eliminar estilos `.factura-local-actions`, `.btn-factura-local-confirmar`, `.btn-factura-local-actual` y `.factura-tag.local`.
4. En `js/webhooks.js`, borrar `WEBHOOK_COMPRAS_REASIGNAR_LOCAL` si no existe otro consumidor.

## 4. Exportación de este parche a otro repositorio

Para mover estos cambios a otro repositorio:

1. Verificar que la consulta de nómina ya entregue `apoyos` junto a `parametros`, `detalle`, `resumen` y `totales`.
2. Confirmar que el módulo de nómina cargue una lista de responsables con `id` y `nombre_completo`, porque esa lista se usa para normalizar `responsable turno` y `apoyo`.
3. Centralizar la URL del webhook de reasignación de compras en el archivo equivalente a `js/webhooks.js`; en este repositorio se usa `WEBHOOK_COMPRAS_REASIGNAR_LOCAL`.
4. Confirmar que el repositorio destino tenga una función equivalente a `listAvailableLocalContexts()` para poblar el selector de locales.
5. Si el backend usa nombres distintos para factura (`uuid`, `prefijo`, `consecutivo`), ajustar solo el payload de `reasignarFacturaLocal()`.
6. Validar que la marca local del botón `Actual` sea suficiente para el flujo destino; si el backend requiere persistencia, conectar ese botón al mismo webhook con una acción `confirmar_actual`.

## 5. Check funcional del parche 2

- Nómina - apoyos web: renderiza tabla de apoyos con nombres limpios y sin tecnicismos.
- Nómina - apoyos Excel: agrega hoja `Apoyos` en el archivo descargable.
- Nómina - apoyos cálculo: horas válidas de apoyo y propinas se suman a ingresos, y se recalculan al editar/validar.
- Compras - pestañas: ahora existen cuatro estados visuales: módulo principal, no corresponde, distribuir locales y revisadas.
- Compras - enviar a local: prepara payload con empresa matriz, tenant origen y destino para el webhook centralizado.
- Compras - actual: retira la factura de la vista de distribución localmente para gestionarla en el módulo principal.
- Pendiente de backend: confirmar el contrato final de `WEBHOOK_COMPRAS_REASIGNAR_LOCAL` y si el botón `Actual` debe persistir en servidor además de ocultarse localmente.
